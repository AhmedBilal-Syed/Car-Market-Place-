...
const MakeSection = () => {
      const MakeSectionCategory = data1.find(c => c.title === "Make");
      const ITEMS_PER_PAGE = 12;
      const [pageIndex, setPageIndex] = useState(0);
    
      const pagedItems = MakeSectionCategory.items.slice(
        pageIndex * ITEMS_PER_PAGE,
        (pageIndex + 1) * ITEMS_PER_PAGE
      );
    
      const pageCount = Math.ceil(MakeSectionCategory.items.length / ITEMS_PER_PAGE);
  ...
  const make = data1.find(c => c.title === "Make");
    ...
    <div className="bg-slate-100 px-4 py-6">
         ...
         <div className="grid grid-cols-6 gap-4 px-2 py-4">
           {pagedItems.map((item, index) => (
             <div
               key={index}
               className={`rounded-[4px] p-[10px] border border-[#e5e5e5] cursor-pointer ${MakeSection.cardClass} flex flex-col items-center`}
